...
- 首页自动显示二维码；扫码立即配对。
- 两台设备打开同一个轻传网站，首页「发现设备」会自动列出同一网络的在线页面。核对对方的本机标识后点击「连接」，无需扫码；使用中的设备不可重复连接，关闭页面后自动移出列表。每个标签页单独显示。
...
发现功能要求所有设备连接到**同一个轻传服务实例**。局域网直连部署按服务端网卡的实际子网掩码分组，本机通过 localhost 打开时也能发现局域网设备。不同电脑各自启动独立 EXE 时，不会跨服务自动发现，请打开其中一台电脑提供的网站地址。

公网部署按相同出口 IP 分组，这只能作为同网的近似判断；共享 NAT 的不同网络可能互相可见，VPN 或不同出口也可能导致同一局域网的设备互相不可见。列表中的设备可以直接发起配对，文件仍须接收方确认；WebRTC 能否直连仍取决于网络条件。

客户端地址默认取实际连接地址，仅信任本机回环代理提供的 `X-Forwarded-For` 最后一跳。宝塔的本机 Nginx 配置可直接使用；如果代理在 Docker 或另一台机器上，请用 `TRUSTED_PROXIES=代理的实际IP` 配置可信代理（多个 IP 用逗号分隔），并确保代理追加或覆盖该请求头。未经信任的转发头不会用于网络分组。
...
端到端测试默认使用 Windows 上的 Microsoft Edge，在 3017 端口启动独立服务；服务测试使用 3018 端口。测试解码真实二维码，通过电脑局域网 IP 打开第二个浏览器会话，校验双向文字、文件字节、零字节文件、拒收、重新扫码与手机布局；发现测试覆盖网络隔离、设备上线/离线、使用中状态、主动连接及重新发现连接。运行需要一个可用 IPv4 局域网网卡。
